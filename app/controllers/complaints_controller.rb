class ComplaintsController < ApplicationController
  ...
  def index
    if params.has_key? (:search)
      ...
      @c = Complaint.all.order("uid DESC")
      @complaints = @c.page(params[:page])
    end
    respond_to do |format|
      format.html
      format.csv { send_data @c.to_csv, filename: "complaints-#{Date.today}.csv" }
    end
  end
  ...
  def update_complaints
   if  params.has_key? (:mechenic_search)
     site_address =  params[:mechenic_search][:site_address]
     fault = params[:fault]
     ...
     site_address =  params[:dealer_search][:site_address]
     fault = params[:fault]
